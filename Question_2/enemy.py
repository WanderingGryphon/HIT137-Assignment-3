...
class Enemy:
    def __init__(self, type, x, y, width, height, health, move_speed):
        ...
        self.walk_right_animation_images = [pygame.image.load(os.path.join('images', f'Walk ({i}).png')) for i in range(1, 7)]
        self.walk_left_animation_images = [pygame.transform.flip(image, True, False) for image in self.walk_right_animation_images]
        self.enemy_appear_images = [pygame.image.load(os.path.join('images', f'Deadge ({i}).png')) for i in range(1, 11)]
        self.fly_left_animation_images = [pygame.image.load(os.path.join('images', f'flying_0{i}.png')) for i in range(1, 3)]
        self.fly_right_animation_images = [pygame.transform.flip(image, True, False) for image in self.fly_left_animation_images]
        self.boss_left_animation_images = [pygame.image.load(os.path.join('images', f'Boss ({i}).png')) for i in range(1, 2)]
        self.boss_right_animation_images = [pygame.transform.flip(image, True, False) for image in self.boss_left_animation_images]
